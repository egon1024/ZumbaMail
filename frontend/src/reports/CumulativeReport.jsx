...
function CumulativeReport() {
    ...
    const generateReport = async () => {
      ...
      try {
        const response = await authFetch(
          `/api/reports/cumulative/?organization_id=${selectedOrgId}&session_id=${selectedSessionId}`
        );
  ...
  const formatReportText = () => {
    ...
    if (reportData.activities.length === 0) {
      ...
      reportData.activities.forEach(activity => {
        lines.push(`${activity.day_of_week} ${activity.class_type}:`);

        if (activity.no_shows.length === 0) {
          lines.push('No absences recorded');
        } else {
          activity.no_shows.forEach(noShow => {
            lines.push(`${noShow.student_name} - ${formatDateWithoutYear(noShow.date)}`);
          });
        }
  ...
  return (
    <div className="container mt-4">
      <h2 className="mb-4" style={{ color: '#6a359c' }}>Cumulative No-Show Report</h2>
        ...
        <div className="card-body">
          ...
          {!selectedOrgId && !loading && !error && (
            <div className="alert alert-secondary mt-3 mb-0" role="alert">
              ...
              Please select an organization and session to view the cumulative no-show report.
            </div>
          )}
